...
private:
  ...
  // Assuming estimated_views is a vector of integers representing the IDs of
  // views that have been estimated
  Eigen::VectorXi estimated_views;

  Eigen::MatrixXd
      cameras; // 6x4 matrix for projective estimation of initial cameras
  Eigen::MatrixXd
      points; // 4xK matrix for projective estimation of initial points
  std::vector<int> pathway; // Order in which views and points are added
  // Assuming 'fixed' is a vector of vectors, representing the points or views
  // used in constraints

  DataStructures::ComputedCameraPoints
  compute_cams(std::vector<int> initial_views, std::vector<int> initial_points,
               Eigen::MatrixXd fundamental_matrix);

public:
  Initialisation(const Eigen::MatrixXd &measurement,
